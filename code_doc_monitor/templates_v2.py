...
* :data:`UNIT_TEMPLATE`     → :func:`code_doc_monitor.config.load_unit_file`
* :data:`INDEX_TEMPLATE`    → :func:`code_doc_monitor.config.load_index_file`
* :data:`IGNORE_TEMPLATE`   → :func:`code_doc_monitor.config.load_ignore_file`
* :data:`DOC_STYLE_TEMPLATE`→ :func:`code_doc_monitor.docstyle.load_doc_style`
...
levels below the repo root, so the ONE :func:`code_doc_monitor.config.resolve_repo_root`
...
version: "2.0.0"
apply_default: false                   # does `cdmon monitor` auto-apply fixes?
...
# (`cdmon index` regenerates this). Reserved stems (index/ignore/doc-style) are
# NOT units.
units:
  - file: example.yaml
...
def scaffold_config_dir(config_dir: Path, *, repo: str, now: str) -> None:
    ...
    :func:`code_doc_monitor.config.load_bundle` for a repo that ships those
    writing templates.

    Loud (K8): the directory is created if absent; an OS error wraps into a typed
    :class:`ConfigError`. Caller (``cdmon init --v2``) enforces the no-clobber
    policy.
    """
    try:
